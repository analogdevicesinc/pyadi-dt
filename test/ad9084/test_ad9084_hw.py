...
import iio
import adijif
from test.dts_utils import (
    download_and_cache_toolchain,
    compile_dts_to_dtb,
    verify_dts_match,
    TOOLCHAIN_2025_R1_ARCH_ARM64,
    compile_kernel,
    add_profile_to_defconfig,
)
...
class TestAD9084Hardware:
    ...
    def test_vpk180_rev10_ad9084(self, kernel_path, dtb_output_dir, post_power_off):
        ...
        config = {
            ...
            "jesd_rx": {"lanerate_khz": 20625000, "link_clk": 312500000, "subclass": 1},
            "jesd_tx": {"lanerate_khz": 20625000, "link_clk": 312500000},
        ...
        generated_dts = board.gen_dt(
            clock=ccfg, adc=adc, dac=dac, fpga=fpga, config_source="manual_config"
        )
        ...
        print("[2.2/4] Updating defconfig...")
        add_profile_to_defconfig(Path(kernel_path), "vpk180", config["device_profile"])

        # Step 2.5: Build kernel
        print("[2.5/4] Building kernel...")
        built_kernel_path = compile_kernel(
            kernel_path, arch="arm64", version="2025.1", platform="vpk180"
        )
        ...
        print("[3/4] Verifying content match...")
    ...
    def test_vpk180_rev10_ad9084_json_profile(
        self, kernel_path, dtb_output_dir, post_power_off
    ):
        ...
        sys.converter.apply_profile_settings(
            str(profile_json), bypass_version_check=True
        )
        ...
        # Cleanup unused
        if "vpk180_adc_device_clk" in clks:
            clks.pop("vpk180_adc_device_clk")
        if "vpk180_dac_ref_clk" in clks:
            clks.pop("vpk180_dac_ref_clk")
        if "vpk180_dac_device_clk" in clks:
            clks.pop("vpk180_dac_device_clk")
        if "vpk180_AD9084_RX_device_clk" in clks:
            clks.pop("vpk180_AD9084_RX_device_clk")

        # Set profile binary name
        cfg["device_profile"] = profile_bin.name

        # Step 2: Generate DTS
        print("[2/4] Generating DTS...")
        board = ad9084_fmc_no_plugin(platform="vpk180", kernel_path=kernel_path)
        ...
        # Generate
        generated_dts = board.gen_dt(
            clock=ccfg, adc=adc, dac=dac, fpga=fpga, config_source="profile_config"
        )
        ...
        print("[2.5/4] Compiling kernel with new firmware binary...")
        ...
        print(
            f"      Copying {Path(kernel_path) / 'arch' / 'arm64' / 'boot' / 'Image'} to {Path(TFTP_BOOT_FOLDER) / 'Image'}"
        )
        shutil.copy2(
            Path(kernel_path) / "arch" / "arm64" / "boot" / "Image",
            Path(TFTP_BOOT_FOLDER) / "Image",
        )

        # Step 3: Compile DTB
        print("[3/4] Compiling DTB...")
