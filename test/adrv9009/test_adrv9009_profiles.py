...
def test_adrv9009_new(target, profile):
    ...
    dts_filename = kernel_path / "arch/arm64/boot/dts/xilinx" / "adrv9009_fmc.dts"
    ...
    if profile_data:
        ...
        if "tx" in profile_data:
            ...
            tx["lpbkAdcProfile"] = profile_data.get("lpbk", {}).get(
                "lpbkAdcProfile", {}
            )
        if "orx" in profile_data:
            orx["profile"] = profile_data["orx"]
            orx["filter"] = profile_data["orx"].get("filter", {})
            orx["orxBandPassAdcProfile"] = profile_data["orx"].get(
                "orxBandPassAdcProfile", {}
            )
            orx["orxLowPassAdcProfile"] = profile_data["orx"].get(
    ...
    if adrv9009_search and len(adrv9009_search) > 0 and adrv9009_search[0]:
        ...
        actual_rx = int(
            shell.run_check(f"cat {adrv9009_phy}/in_voltage0_sampling_frequency")[0]
        )
        actual_tx = int(
            shell.run_check(f"cat {adrv9009_phy}/out_voltage_sampling_frequency")[0]
        )

        expected_rx_rate = (
            int(float(profile_data["rx"].get("rxOutputRate_kHz", 245760))) * 1000
        )
        expected_tx_rate = (
            int(float(profile_data["tx"].get("txInputRate_kHz", 245760))) * 1000
        )
