...
2. Deployed to a running ZCU102 booted with pipeline-generated merged DTB
3. Loaded at runtime via configfs (/sys/kernel/config/device-tree/overlays/)
4. Verified via pylibiio that IIO devices appear (after iiod restart)
5. Unloaded by removing the configfs overlay directory
6. Verified that IIO devices disappear after iiod restart
7. Reloaded to confirm repeatability
8. JESD204 links in DATA mode
9. DMA TX→RX loopback data captured

Prerequisites:
    - LG_ENV=/jenkins/lg_ad9081_zcu102.yaml
    - sdtgen and dtc on PATH
    - ZCU102 running Kuiper Linux with CONFIG_OF_OVERLAY=y
    - M8/L4 BOOT.BIN for 122.88 MHz VCXO on SD card
"""
...
def test_unload_overlay(booted_board):
    """Unload overlay, restart iiod, verify IIO devices removed via pylibiio."""
    import iio
...
def test_reload_overlay(booted_board, overlay_dtbo):
    """Reload overlay after unload, verify via pylibiio."""
    import iio
