...
#### Fixed-JTAG ZynqMP boards

The ADRV9009-ZU11EG is physically strapped for JTAG, so it cannot use the
generic ``boot_mode="sd"`` transport.  Its hardware test uses the dedicated
``test/hw/config/zu11eg-jtag.yaml`` environment and the merged
``BootZynqMPJTAG`` strategy.  The test is still end-to-end and validates the
DTB generated by the current checkout:

1. Generate and compile the ZU11EG DTB from the committed XSA.  The built-in
   profile carries the production 245.76 MSPS Talise properties and physical
   Rev.B wiring (PHY0/PHY1/HMC7044 on SPI chip selects 0/1/2, reset GPIOs
   130/156, and SYSREF request GPIO 167).
2. Apply board facts that are not encoded correctly in the XSA: install the
   SD1 pinmux, constrain it to the physical four-bit interface by removing
   SDT's erroneous ``xlnx,bus-width = <8>``, disable 1.8 V switching/write
   protect, and select MIO bank 1.
3. Reconstruct the production PMUFW/PL/BL31/U-Boot handoff through JTAG and
   boot the known-good Kuiper SD DTB.
4. Transfer the generated DTB over the serial shell, remove SDT's fixed
   ``/chosen/bootargs`` so production U-Boot supplies Kuiper's SD-root and
   console arguments, preserve the production ``system.dtb`` as
   ``system.dtb.pyadi-dt-backup``, and install the generated file atomically.
5. Power-cycle and repeat the complete JTAG handoff.
6. Require a Kuiper shell, clean kernel/probe logs, two Talise JESD FSM
   initializations, RX/observation-RX/TX JESD links in DATA, both ADRV9009 PHY
   IIO devices, and a nonconstant RX capture.
7. Restore the production DTB and perform one final production Kuiper/JESD/IIO
   boot before powering the board down.

The CI runner must stage the immutable payloads referenced by
``test/hw/config/zu11eg-jtag.yaml`` under
``~/.cache/adidt-ci/zu11eg-recovery/``.  These are board-specific artifacts
extracted or built by the reproducible recipes in adi-labgrid-plugins; they
must not be committed to pyadi-dt.  The labgrid plugin dependency is pinned to
the merge commit that introduced ``BootZynqMPJTAG``, and the persistent venv
installer forcibly refreshes that package because its Python package version
does not change between plugin commits.  CI runs
``.github/scripts/verify-zu11eg-jtag-artifacts.sh`` before acquisition; the
checked-in SHA-256 manifest rejects missing, substituted, or stale payloads.

Run the physical test on the runner that has Vitis and the staged payloads:

```sh
curl -fsSL "http://10.0.0.41:8000/api/places/tron/env-yaml?tier=boot" >/dev/null

LG_COORDINATOR=10.0.0.41:20408 \
LG_ENV="$PWD/test/hw/config/zu11eg-jtag.yaml" \
LG_FORCE_LOCAL_XSDB=1 \
uv run pytest -p no:genalyzer -v -s \
  test/hw/test_adrv9009zu11eg_adrv2crr-fmc_hw.py
```

If the generated DTB cannot reach Linux, do not re-image the SD card.  Enter
the strategy's RAM-rooted ``recovery_linux`` state, mount ``/dev/mmcblk0p1``,
and restore ``system.dtb.pyadi-dt-backup`` to ``system.dtb``.  This backup is
created before every generated-DTB boot specifically to keep the test
recoverable without removing the SD card.
