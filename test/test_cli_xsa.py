...
def test_xsa2dt_warns_non_path_parity_artifacts_in_strict_mode(tmp_path):
    runner = CliRunner()
    xsa = tmp_path / "design.xsa"
    cfg = tmp_path / "cfg.json"
    out = tmp_path / "out"
...
# ---------------------------------------------------------------------------
# --format petalinux + --petalinux-project install flow
# ---------------------------------------------------------------------------


def _make_petalinux_project_skeleton(root):
    """Create the minimum directory layout that ``validate_petalinux_project``
    expects: ``project-spec/meta-user/recipes-bsp/device-tree/files``.
    Mirrors the helper used by ``test/xsa/test_petalinux.py``.
    """
    dt_files = (
        root
        / "project-spec"
...
def _write_pipeline_artifacts(out_dir):
    """Write minimal pipeline outputs that the CLI install path needs to
    exist on disk before copying them into the PetaLinux project."""
    out_dir.mkdir(parents=True, exist_ok=True)
    overlay = out_dir / "a.dtso"
    merged = out_dir / "a.dts"
    sysuser = out_dir / "system-user.dtsi"
    bbappend = out_dir / "device-tree.bbappend"
    overlay.write_text("/dts-v1/;\n/plugin/;\n")
    merged.write_text("/dts-v1/;\n/ { };\n")
    sysuser.write_text("/* generated by pyadi-dt */\n")
    bbappend.write_text('FILESEXTRAPATHS:prepend := "${THISDIR}/files:"\n')
    return overlay, merged, sysuser, bbappend


def test_xsa2dt_petalinux_project_copies_dtsi_and_bbappend(tmp_path):
    runner = CliRunner()
    xsa = tmp_path / "design.xsa"
    cfg = tmp_path / "cfg.json"
    out = tmp_path / "out"
    project = tmp_path / "myproject"
    dt_files = _make_petalinux_project_skeleton(project)
    xsa.write_bytes(b"PK\x03\x04")
    cfg.write_text(
        json.dumps({"jesd": {"rx": {"F": 4, "K": 32}, "tx": {"F": 4, "K": 32}}})
    )

    overlay, merged, sysuser, bbappend = _write_pipeline_artifacts(out)

    with patch("adidt.xsa.pipeline.XsaPipeline") as MockPipeline:
        MockPipeline.return_value.run.return_value = {
            "overlay": overlay,
            "merged": merged,
            "system_user_dtsi": sysuser,
            "bbappend": bbappend,
            "base_dir": out / "base",
        }
        result = runner.invoke(
    ...
    assert result.exit_code == 0, result.output
    assert (dt_files / "system-user.dtsi").exists()
    assert (dt_files.parent / "device-tree.bbappend").exists()
    assert "Installed into PetaLinux project" in result.output


def test_xsa2dt_petalinux_project_backs_up_existing_dtsi(tmp_path):
    runner = CliRunner()
    xsa = tmp_path / "design.xsa"
    cfg = tmp_path / "cfg.json"
    out = tmp_path / "out"
    ...
    # Pre-existing system-user.dtsi must be backed up before the new copy lands.
    existing = dt_files / "system-user.dtsi"
    existing.write_text("/* user's previous content */\n")
    ...
    assert backup.exists(), "expected pre-existing dtsi to be backed up"
    assert "user's previous content" in backup.read_text()
    assert "Backed up existing system-user.dtsi" in result.output


def test_xsa2dt_petalinux_project_rejects_invalid_project(tmp_path):
    runner = CliRunner()
    xsa = tmp_path / "design.xsa"
    cfg = tmp_path / "cfg.json"
    out = tmp_path / "out"
    project = tmp_path / "not_a_project"
    project.mkdir()  # exists but lacks the expected layout
    xsa.write_bytes(b"PK\x03\x04")
    cfg.write_text(
        json.dumps({"jesd": {"rx": {"F": 4, "K": 32}, "tx": {"F": 4, "K": 32}}})
    )
    ...
    assert "Not a valid PetaLinux project" in combined
