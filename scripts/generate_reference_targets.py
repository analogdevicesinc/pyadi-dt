...
class DTSAnalyzer:
    ...
    def identify_pll_config(self, dts_content: str, board: str) -> str | Dict[str, str]:
        ...
        if board == "adrv9009":
            return {"rx": "XCVR_CPLL", "tx": "XCVR_QPLL", "orx": "XCVR_CPLL"}
    ...
    def find_template(self, board: str, platform: str) -> Optional[str]:
        ...
        for template_name in patterns:
            template_path = (
                self.linux_path.parent / "adidt" / "templates" / template_name
            )
    ...
    def find_additional_variants(
        self, dts_path: str, board: str, platform: str
    ) -> List[str]:
        ...
        if os.path.exists(dts_dir):
            for filename in os.listdir(dts_dir):
                ...
                if (
                    board.replace("_", "") in filename.lower()
                    and platform in filename.lower()
                ):
    ...
    def generate_targets_json(
        self, dts_files: List[str], output_path: Path, merge: bool = False
    ) -> None:
    ...
    def _merge_json_data(self, existing: Dict, new: Dict) -> Dict:
        ...
        for board_name, new_board_data in new["boards"].items():
            if board_name not in merged["boards"]:
                ...
                existing_board["description"] = new_board_data.get(
                    "description", existing_board.get("description")
                )
                ...
                for platform_name, new_platform_data in new_board_data[
                    "platforms"
                ].items():
                    if platform_name not in existing_board["platforms"]:
                        ...
                        existing_variants = set(
                            existing_platform.get("additional_variants", [])
                        )
                        new_variants = set(
                            new_platform_data.get("additional_variants", [])
                        )
