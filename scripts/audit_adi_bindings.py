#!/usr/bin/env python3
"""Audit ADI-compatible bindings in Linux against known pyadi-dt compatibles."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Find ADI devicetree bindings in Linux that are not present in"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Exit non-zero when undocumented bindings exist.",
    )
...
def main() -> int:
    ...
    try:
        ...
        # Keep supported list aligned for transparency and easier CI diagnostics.
        known_compatibles = collect_supported_compatibles(args.project_root)
        known_prefixes = collect_supported_prefixes(known_compatibles)
        if known_compatibles:
            report["known_compatibles"] = sorted(known_compatibles)
            report["known_prefixes"] = sorted(known_prefixes)

        if args.output:
            write_json(args.output, report)
        if args.report:
            write_markdown(args.report, report, kind="audit")
        ...
        if args.fail_on_undocumented and report["summary"]["undocumented_bindings"] > 0:
            return 2
        return 0
    finally:
