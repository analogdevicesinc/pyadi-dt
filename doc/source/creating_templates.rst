...
.. code-block:: text
   ...
   Linux kernel source
       │
   scripts/collect_adi_bindings.py
       │  parses YAML/TXT bindings
       ▼
   Audit report (known / undocumented compatibles)
       │
   scripts/audit_adi_bindings.py --generate-templates
       │  generates starter .tmpl stubs
       ▼
   adidt/templates/xsa/<device>.tmpl  (starter stub)
       │
   Manual editing (add properties, clocks, GPIOs)
       │
       ▼
   adidt/model/contexts.py  (context builder function)
       │
   adidt/model/components.py  (factory function)
       │
       ▼
   Ready to use in BoardModel
